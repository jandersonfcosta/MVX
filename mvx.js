/*
    ...
    Descrição: Uma simples API MVVM

    Uso: mvx.bind() - ver demo.html
*/
...
function MVX() {
	var ATTRIBUTES = "[v-value], [v-text], [v-repeat], [v-hide], [v-show], [v-disabled], [v-enabled], [v-class]",
		DATA = window,
		BINDED; // elementos que foram vinculados


	// PÚBLICO

	this.bind = function(elements, callback) {
		BINDED = [];
		cleanRepeatClones(document);

		if (typeof elements !== "function")
			bind(DATA, elements, callback);
		else
			bind(DATA, null, elements);
	};
	...
	function bind(data, elements, callback, canBind) {
	...
	function repeatBind(template) {
		...
		// <select/>
		if (tagName === "OPTION")
			bind(DATA, [element.parentNode], null, true);
	}
	...
	function vClass(element, data) {
		...
		if (attr && !isBinded(element, attrName)) {
			var values = attr.value.split(":"),
				_data = values[0].trim(),
				classes = values[1].trim().replace(/  /g, "").split(" "),
